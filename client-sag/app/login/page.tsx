...
export default function LoginPage() {
  ...
  // --- FUNGSI LOGIN DENGAN INTEGRASI API LARAVEL ---
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);

    try {
      // Pastikan server Laravel kamu jalan di http://localhost:8000
      ...
      if (response.ok) {
        // 1. Simpan Token Sanctum ke localStorage
        ...
        // 3. Logika Remember Me (simpan email saja)
        if (rememberMe) {
      ...
      alert("Tidak dapat terhubung ke server. Pastikan API Laravel sudah dijalankan (php artisan serve).");
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white font-jakarta flex overflow-hidden">
      ...
      <div className="w-full lg:w-[45%] min-h-screen flex items-center justify-center p-6 sm:p-12 lg:p-20 bg-white relative">
        
        <Link href="/" className="absolute top-6 left-6 lg:top-10 lg:left-10 z-20 group">
          <div className="flex items-center gap-3 px-4 py-2 rounded-full bg-gray-50 border border-gray-100 text-gray-500 transition-all duration-300 group-hover:bg-white group-hover:border-blue-200 group-hover:shadow-md group-hover:text-sag-blue">
        ...
        <div className="w-full max-w-sm" data-aos="fade-right">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div className="text-left">
              <label className="block text-[11px] font-black uppercase tracking-widest text-gray-400 mb-2 ml-1">Email Administrator</label>
              <input 
                type="email" 
                value={email}
            ...
            <div className="relative text-left">
              <div className="flex justify-between items-center mb-2 ml-1">
                <label className="text-[11px] font-black uppercase tracking-widest text-gray-400">Security Password</label>
              </div>
            ...
            <div className="flex items-center justify-between">
              <button type="button" onClick={() => setRememberMe(!rememberMe)} className="flex items-center gap-2 cursor-pointer group">
              ...
              <Link href="#" className="text-xs font-bold text-blue-600 uppercase tracking-tighter hover:underline text-right">Lupa Password?</Link>
            </div>

            <button type="submit" disabled={loading} className="w-full bg-sag-blue text-white py-4.5 rounded-full font-black uppercase tracking-widest shadow-xl shadow-blue-200 hover:bg-blue-800 transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3">
              {loading ? <div className="w-5 h-5 border-3 border-white/20 border-t-white rounded-full animate-spin" /> : <span>Masuk</span>}
            </button>
          </form>
        </div>

        <div className="absolute bottom-8 left-6 lg:left-10 text-gray-300 text-[9px] font-black uppercase tracking-[0.3em]">
      ...
      <div className="hidden lg:flex lg:w-[55%] bg-gradient-to-br from-sag-blue via-[#1e40af] to-[#1e3a8a] relative p-16 flex-col justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-xl pt-10 text-left" data-aos="fade-left">
          <div className="flex items-center gap-6 mb-10 mt-4 brightness-0 invert opacity-90 drop-shadow-xl">
          ...
          <h2 className="text-white text-6xl font-black uppercase tracking-tighter leading-[0.9] mb-8">
            Innovating System, <br /> <span className="text-blue-400">Empowering Tomorrow.</span>
          </h2>
          
          <p className="text-blue-100/70 text-lg font-medium leading-relaxed mb-12">
